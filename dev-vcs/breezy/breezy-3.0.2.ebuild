...
DESCRIPTION="Breezy is a version control system implemented in Python with multi-format support and an emphasis on hackability."
...
SRC_URI="https://github.com/breezy-team/breezy/archive/${PV}-${R_VER}.tar.gz"

RDEPEND="curl? ( dev-python/pycurl[${PYTHON_USEDEP}] )
	sftp? ( dev-python/paramiko[${PYTHON_USEDEP}] )"

DEPEND="!!dev-vcs/bzr
	|| ( dev-python/cython[${PYTHON_USEDEP}] dev-python/pyrex[${PYTHON_USEDEP}] )
	test? ( ${RDEPEND}
...
python_test() {
	...
	LC_ALL="C" "${PYTHON}" brz --no-plugins selftest -v \
