...
EAPI=5
inherit eutils systemd toolchain-funcs udev flag-o-matic
...
src_prepare() {
	...
	sed -i -e '/sys\/types\.h/s/$/\n#include <sys\/sysmacros\.h>/g' $(find -type f -name '*.c' -o -name '*.h') || die

	sed -i -e 's/no-typedef-redefinition/no-error=typedef-redefinition/g' \
		configure*
}
...
src_configure() {
	...
	append-cppflags -DHAVE_MEMFD_CREATE
