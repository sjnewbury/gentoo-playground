...
inherit eutils java-pkg-2 java-ant-2 multilib flag-o-matic systemd
...
src_install() {
	...
	cp -r "${D}"/usr/"$(get_libdir)"/"${P}"/docs ${D}/"${I2P_BASE_DIR}"/docs
...
pkg_preinst() {
	enewgroup i2p
	enewuser i2p -1 -1 /var/lib/${PN} i2p -m
}
