...
src_prepare() {
	...
	# llvm-config should return shared libs not non-existant static
	# archives
	epatch "${FILESDIR}/${P}-shared-libnames.patch"
...
#	fi
	...
	if multilib_is_native_abi; then
		...
		if use gold; then
			binutils_active_profile=$(binutils-config -c)
			binutils_cname=${binutils_active_profile%-*}
			binutils_version=${binutils_active_profile/${binutils_cname}-}
			mycmakeargs+=(
				-DLLVM_BINUTILS_INCDIR="${EPREFIX}"/usr/$(get_libdir)/binutils/"${binutils_cname}"/"${binutils_version}"/include
			)
		fi
