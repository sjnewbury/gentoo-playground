...
# $Header: /var/cvsroot/gentoo-x86/net-libs/libsoup-gnome/libsoup-gnome-2.46.0.ebuild,v 1.7 2014/08/25 11:03:01 ago Exp $
...
if [[ ${PV} = 9999 ]]; then
	inherit gnome2-live
	EGIT_REPO_URI="${EGIT_REPO_URI/-gnome}"
	EGIT_CHECKOUT_DIR="${WORKDIR}/${P/-gnome}"
fi
...
src_prepare() {
	...
	if [[ ${PV} = 9999 ]]; then
		epatch "${FILESDIR}"/${PN}-9999-glib-enums.patch
	else
		eautoreconf
	fi
	gnome2_src_prepare
