...
#		"${FILESDIR}/${PN}-optional-PIE.patch"
	...
	# Regen driver blacklist after patching
	gpu/config/process_json.py || die
	...
	# Support no-PIE/no-PIC (flags re-added later if use pie)
	#sed -e '/-fPIC/d' \
	#	-i build/config/compiler/BUILD.gn

	#sed -e '/-pie/d' \
	#	-i chrome/BUILD.gn v8/gni/v8.gni
...
src_configure() {
	...
	if use system-ffmpeg; then
		# Disable OpenH264 support since we're using ffmpeg/libva
...
src_install() {
	...
	dosym "${CHROMIUM_HOME}/chromium-launcher.sh" /usr/bin/chromium-browser${CHROMIUM_SUFFIX}
	# keep the old symlink around for consistency
	dosym "${CHROMIUM_HOME}/chromium-launcher.sh" /usr/bin/chromium${CHROMIUM_SUFFIX}

	dosym "${CHROMIUM_HOME}/chromedriver" /usr/bin/chromedriver${CHROMIUM_SUFFIX}
