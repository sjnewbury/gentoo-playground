...
SRC_URI="
		https://commondatastorage.googleapis.com/chromium-browser-official/${P}-lite.tar.xz
"
#		wayland? ( https://github.com/01org/ozone-wayland/archive/${PV}.tar.gz )
...
src_unpack() {
	unpack "${A}"
	if use wayland; then
		EGIT_CHECKOUT_DIR="${WORKDIR}/ozone-wayland-${PV}" git-r3_src_unpack
	fi
}
...
src_prepare() {
	...
	build/linux/unbundle/remove_bundled_libraries.py \
		...
		# Import ozone and patch sources
		if use wayland; then
			cp -al "${WORKDIR}/ozone-wayland-${PV}" "${S}"/ozone || die
			
			# Upstreamed?
			rm -f ./ozone/patches/1001-WebRTC-Remove-Dependencies-on-X11.patch

			EPATCH_SUFFIX="patch" EPATCH_FORCE="yes" \
				epatch ./ozone/patches/ || die

			# Local ozone patches
			#cd ozone
			#epatch "${FILESDIR}/${PN}-ozone-HWA-decode.patch"
			#cd ..
		fi
}
...
src_configure() {
	...
	use sse2 || myconf+=" -Ddisable_sse2=1"
	...
	myconf+="
		...
		$(gyp_use wayland use_ozone)
		$(gyp_use wayland enable_xdg_shell)"
