...
src_prepare() {
	...
	if [[ ${PV} == 9999 ]]; then
		rm -v "${WORKDIR}"/firefox-patches/*-bmo-1715254-Deny-clone3-to-force-glibc-fallback.patch
		rm -v "${WORKDIR}"/firefox-patches/*-bmo-1719674-Make-packed_simd-compile-with-Rust-*.patch
	fi
...
src_configure() {
	...
	[[ ${PV} == 9999* ]] && update_channel=nightly
