...
IUSE="context debug doc icu +nls mpi python static-libs +threads tools avx2"
...
src_prepare() {
	...
	# This is not the only project that has deciced that users of new modern
	# gcc versions unconditionally support AVX2 instructions... WTF?
	use avx2 || sed -i \
		-e 's:dump_avx2::g' \
		-e 's:<define>BOOST_LOG_USE_AVX2::g' \
		libs/log/build/Jamfile.v2 || die Failed to disable AVX2 support
